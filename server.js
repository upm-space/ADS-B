...
var client = net.connect({host: host, port: port}, function () {
    ...
    hora_inicial = fecha_inicial.getTime();
});
...
function decoder(paquetes){
    ...
    if (downlink_format == 17) {
        ...
        var flight_id = [];
        if (type_code >= 1 && type_code <= 4) {
            flag_datos_coordenadas = false;
            flag_datos_callsign= true;
            flag_datos_velocidad = false;
        ...
        // Posición (Baro-altitud)
            ...
            flag_datos_coordenadas = true;
            flag_datos_callsign= false;
            flag_datos_velocidad = false;
            fflag = bytes[6] & (1 << 2);
            tflag = bytes[6] & (1 << 3);
            ...
            if (q_bit) {
                ...
                if (even_odd == 0) { // Even
                    ...
                    var NZ = 15;
                    var NL = Math.floor((2 * Math.PI) / (Math.acos(1 - (1 - Math.cos(Math.PI / (2 * NZ))) / (Math.pow(Math.cos((latitud_decode * Math.PI) / (180)), 2)))));
...
var numero_vuelo_repetido = 0;

// Funcion para almacenar los datos
function save_data() {
    fecha_mensaje = new Date();
    hora_mensaje = fecha_mensaje.getTime();
    console.log(hora_mensaje);
    ...
    vector_vuelos[8] = hora_mensaje - hora_inicial;
    ...
    // Para los siguientes vuelos se comprueba si el vuelo es nuevo o no con el código OACI
        ...
        if (flag_repetido == false) {
            ...
            vector_datos.push(vector_vuelos.slice());
            numero_vuelo_repetido = vector_datos.length - 1;
        }

        console.log("REPETIDO: " + flag_repetido);
        console.log("COORDENADAS: " + flag_datos_coordenadas);
        console.log("NUMERO: " + numero_vuelo_repetido);

        sendData_info_coordenada();
        sendData_info_callsign();
        sendData_info_velocidad();
        sendData_info_numero_vuelo_();
        sendData_adsb();
    }
}
...
// Envía un mensaje al conectarse por primera vez
io.sockets.on('connection', function (socket) {
    socket.emit('welcome', { message: 'Conexión establecida', id: socket.id });
});

// Envía mensaje cuando se llama a esta función. En este caso el vector de vuelos
function sendData_adsb() {
    io.emit('adsb_data', { adsb_data: vector_datos});
    console.log("ADSB: " + vector_datos);
}

function sendData_info_coordenada() {
    io.emit('infro_coordenada', { infro_coordenada: flag_datos_coordenadas});
}

function sendData_info_callsign() {
    io.emit('infro_callsign', { infro_callsign: flag_datos_callsign});
}

function sendData_info_velocidad() {
    io.emit('infro_velocidad', { infro_velocidad: flag_datos_velocidad});
}

function sendData_info_numero_vuelo_(){
    io.emit('numero_vuelo', { numero_vuelo: numero_vuelo_repetido});
}
